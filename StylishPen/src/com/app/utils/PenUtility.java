...
public class PenUtility {
	...
	public static void removePen(List<Pen> penList) {
		...
		while(itr.hasNext()) {
			int year=Period.between(itr.next().getUpdateDate(),LocalDate.now()).getYears();
			if(Period.between(itr.next().getUpdateDate(),LocalDate.now()).getMonths()+year*12>=9)
			{
				itr.remove();
			}
		}
//		for (Pen pen : penList) {
//			if(Period.between(pen.getUpdateDate(),LocalDate.now()).getMonths()>=9) {
//				
//			}
	}
}
